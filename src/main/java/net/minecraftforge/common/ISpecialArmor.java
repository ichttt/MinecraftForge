...
public interface ISpecialArmor
{
     ...
     */
    ArmorProperties getProperties(EntityLivingBase entity, @Nonnull ItemStack armor, DamageSource source, double damage, EntityEquipmentSlot slot);
     ...
     */
    int getArmorDisplay(EntityPlayer player, @Nonnull ItemStack armor, EntityEquipmentSlot slot);
     ...
     * @param slot The armor slot the item is in.
     */
    void damageArmor(EntityLivingBase entity, @Nonnull ItemStack stack, DamageSource source, int damage, EntityEquipmentSlot slot);

    class ArmorProperties implements Comparable<ArmorProperties>
    {
        // Only enable this if you wish to be spammed with debugging information.
        // Left it in because I figured it'd be useful for modders developing custom armor.
        private static final boolean DEBUG = Boolean.parseBoolean(System.getProperty("forge.debugSpecialArmor", "false"));

        public int priority;
        public int absorbMax;
        public double absorbRatio;
        public double armor; // Additional armor, separate from the armor added by vanilla attributes.
        public double toughness; // Additional toughness, separate from the armor added by vanilla attributes.
        ...
        public static float applyArmor(EntityLivingBase entity, DamageSource source, double damage)
        {
            ...
            if (dmgVals.size() > 0)
            {
                ...
                for (ArmorProperties prop : props)
                {
                    ...
                    if (absorb > 0)
                    {
                        ItemStack stack = entity.getItemStackFromSlot(prop.slot);
            ...
            if (damage > 0 && (totalArmor > 0 || totalToughness > 0))
            {
                ...
                for (EntityEquipmentSlot slot : ARMOR_SLOTS)
                {
                    ...
                    {
                        stack.damageItem((int) armorDamage, entity);
                    }
                }
        ...
        @Override
        public int compareTo(@Nonnull ArmorProperties o)
        {
